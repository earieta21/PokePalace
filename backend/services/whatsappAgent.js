/* Agente conversacional de WhatsApp — toma pedidos del menú fijo de apertura.
   ...
   resuelven contra POS_CATALOG en el servidor — el modelo solo elige
...
import { isWithinRestaurantHours } from "../utils/customerOrder.js";
import { TAX_RATE } from "../pricing.js";
...
// Menú vendible por WhatsApp: los 4 bowls de la casa + starters + bebidas.
// Excluye "bowl-mediano-rapido"/"bowl-grande-rapido" (venta exprés del POS
// sin receta conocida, pensada para la fila del mostrador, no para un chat).
const MENU_CATALOG_IDS = [
  "bowl-emerald-salmon", "bowl-spicy-tuna", "bowl-tropical-shrimp", "bowl-citrus-tofu",
  "edamame", "seaweed-salad",
  "mineral-water", "coca-zero", "bottled-water", "agua-del-dia",
];
...
async function confirmOrder(session, phone, nombreCliente) {
  ...
  if (!isWithinRestaurantHours(new Date())) {
    return { error: true, message: "Estamos cerrados ahora. Abrimos de jueves a martes, 11:00 a 21:00." };
  }

  const cartItems = session.cart.map((c) => ({ catalogId: c.catalogId, qty: c.qty }));
  const unavailableNow = getUnavailablePosSelections({ items: cartItems, unavailableItems: settings?.unavailableItems || [] });
  ...
  let safeItems;
  try {
    safeItems = resolvePosItems(cartItems);
  } catch {
